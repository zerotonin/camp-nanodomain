#!/usr/bin/env python3
"""
cAMP Nanodomain Model — GPU-Accelerated Parameter Optimisation v4
==================================================================

Updates from v3:
  - Rapid Buffering Approximation (RBA) for Ca²⁺:
      κ_S = 77 (Bhatt & Bhatt 2011, Drosophila Ib motor terminal)
      τ_Ca = 46 ms (measured free-Ca²⁺ decay in same study)
      Only 1/(1+κ_S) ≈ 1.3% of entering Ca²⁺ remains free.
      This is the standard approach when endogenous buffers are fast
      relative to other fluxes (Smith et al. 1996, Wagner & Keizer 1994).

  - Dual Ca²⁺ input:
      * Odour → KC depolarisation → sustained Ca²⁺ influx (all boutons)
      * Shock → circuit reverberation / field effect → brief Ca²⁺ transient
      Both scaled by 1/(1+κ_S) via RBA.
      With τ_Ca = 46 ms, each shock produces a sharp, isolated Ca²⁺ peak
      that decays well before the next shock (5 s ITI), giving the
      sawtooth/staircase pattern visible in raw data.

  - Two-term AC production (from v3):
      * V_Ca  × f(Ca) × odor_on     [all compartments, small]
      * V_coinc × f(Ca) × DA_mask × shock_on  [γ1-γ3, supralinear]
      DA mask: [1.0, 1.0, 0.5, 0.0, 0.0] (PPL1 aversive DANs)

  - Parameter count: 11 free (same as v3)
      Replaced: Ca_amplitude, tau_Ca → Ca_odor, Ca_shock
      Fixed: τ_Ca = 0.046 s, κ_S = 77 (both from literature)

Usage:
    python optimise_model_gpu_v4.py --data all_camp_long.csv --outdir results_v4/
    python optimise_model_gpu_v4.py --data all_camp_long.csv --quick
    python optimise_model_gpu_v4.py --plot-only results_v4/best_params.json
"""
...
# 11 free parameters (same count as v3)
# Replaced Ca_amplitude + tau_Ca with Ca_odor + Ca_shock
# tau_Ca and kappa_S are now FIXED from Bhatt & Bhatt 2011
...
FIT_BOUNDS = jnp.array([
    ...
    [0.5,    50.0],    # Ca_odor       total Ca²⁺ rate during odour
                       #   RBA makes free [Ca²⁺] = Ca_odor/(1+77) ≈ Ca_odor/78
                       #   So Ca_odor=50 → ~0.64 μM/s free influx rate
    [1.0,   200.0],    # Ca_shock      total Ca²⁺ rate during shock
                       #   Brief 1s pulse: peak free ΔCa ≈ Ca_shock/(1+77) × τ_Ca
                       #   Ca_shock=100 → ~0.06 μM free peak
...
def plot_diagnostic(params_array, data_full, pairing_onsets, geom,
    ...
    fig.suptitle('cAMP Model v4 — RBA Ca²⁺ + Two-Term AC + DA Mask', fontsize=13, fontweight='bold')
...
def main():
    parser = argparse.ArgumentParser(
        description='GPU cAMP model optimisation v4 (RBA Ca²⁺ + two-term AC)')
    ...
    print(f"\n{'='*60}")
    print(f"GPU DIFFERENTIAL EVOLUTION v4 — RBA Ca²⁺ + Two-Term AC")
    print(f"{'='*60}")
    ...
    result = {
        ...
        'model_version': 'v4_RBA_calcium_two_term_AC',
    }
