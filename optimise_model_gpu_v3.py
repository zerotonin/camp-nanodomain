#!/usr/bin/env python3
"""
cAMP Nanodomain Model — GPU-Accelerated Parameter Optimisation v3
==================================================================

Updates from v2:
  - Two-term AC production: Ca²⁺-only (V_Ca, all compartments) +
    coincidence (V_coinc, DA-mask-gated).  This captures the biology:
      * Odour → KC depolarisation → Ca²⁺/CaM → partial rutabaga (everywhere)
      * Shock → DANs → dopamine → Gαs → rutabaga boost (γ1-γ2 only)
      * Both inputs together → supralinear coincidence detection
  - Compartment-specific DA mask: [1.0, 1.0, 0.5, 0.0, 0.0] for γ1-γ5,
    based on aversive PPL1 DAN innervation (Aso et al. 2014).
    γ3 receives mixed input; γ4-γ5 innervated by appetitive PAM DANs
    (not activated by aversive shock).
  - Ca²⁺ influx now driven by ODOUR onset (KC depolarisation), not shock.
    The shock activates DANs, not KCs directly.
  - Tighter G-Flamp1 sensor bounds (τ_sensor: 0.5–10 s) to preserve
    individual shock bumps visible in raw data.

Usage:
    python optimise_model_gpu_v3.py --data all_camp_long.csv --outdir results_v3/
    python optimise_model_gpu_v3.py --data all_camp_long.csv --quick
    python optimise_model_gpu_v3.py --plot-only results_v3/best_params.json
"""
...
def build_geometry(n_comp=5, d_bouton=2.0, d_axon=0.3,
    ...
    # ALL boutons receive Ca²⁺ during odour (5-HT1B-GAL4 protocol)
    stim_mask = bouton_mask.copy()   # 1.0 for all boutons

    # DA mask: compartment-specific DAN innervation during aversive shock
    # γ1, γ2: PPL1 DANs (aversive) → full DA release
    # γ3: mixed PPL1/PAM input → partial
    # γ4, γ5: PAM DANs (appetitive) → no activation by aversive shock
...
PARAM_NAMES = [
    ...
    'k_on_buffer',   # 3   μM⁻¹s⁻¹
    'V_basal',       # 4   μM/s
    'V_Ca',          # 5   μM/s  ← Ca²⁺-only rutabaga activation rate
    'K_Ca',          # 6   μM
    'n_Hill_AC',     # 7
    'k_cat',         # 8   s⁻¹
    'K_m',           # 9   μM
    ...
    'dff_offset',    # 16
    'tau_sensor',    # 17  s
    'V_coinc',       # 18  μM/s  ← Coincidence (Ca²⁺+DA) supralinear rate
]
...
DEFAULTS = jnp.array([
    ...
    10.0,    # k_on_buffer
    0.5,     # V_basal
    2.0,     # V_Ca           ← small: Ca²⁺-only produces modest cAMP
    0.5,     # K_Ca
    2.0,     # n_Hill_AC
    5.0,     # k_cat
    2.4,     # K_m
    ...
    0.1,     # alpha
    0.0,     # dff_offset
    3.0,     # tau_sensor     ← tighter default
    15.0,    # V_coinc        ← large: coincidence is supralinear
])
...
FIT_BOUNDS = jnp.array([
    [0.01,    5.0],    # V_basal
    [0.1,    10.0],    # V_Ca         ← Ca²⁺-only: small but non-zero
    [1.0,   100.0],    # V_coinc      ← coincidence: much larger
    [0.5,   200.0],    # k_cat
    [0.1,    10.0],    # PDE_conc
    ...
    [0.001,   2.0],    # alpha
    [-0.5,    0.5],    # dff_offset
    [0.5,    10.0],    # tau_sensor   ← TIGHTENED: was [0.5, 30]
                       #   G-Flamp1 in vitro: t_on≈0.20s, t_off≈0.09s
                       #   In vivo slower but should preserve shock bumps
                       #   visible in raw data at 5s ITI
])
...
def make_stim_signals(pairing_onsets, odor_dur=5.0, shock_offset=4.0,
    ...
    Each pairing: odour ON for odor_dur seconds, shock arrives at
    shock_offset into the odour.

    In our experiment:
      - Odour → KC depolarisation → Ca²⁺ (all compartments)
      - Shock → DAN activation → DA release (compartment-specific via DA mask)
    """
...
def make_ode_term(geom, params, odor_fn, shock_fn, is_dunce):
    ...
    AC production has two terms:
      1. Ca²⁺-only:     V_Ca   × f(Ca) × odor_on     [all boutons]
      2. Coincidence:    V_coinc × f(Ca) × DA_mask × shock_on  [γ1-γ3 only]

    Ca²⁺ influx is driven by ODOUR (KC depolarisation), not shock.
    Shock activates DANs → DA release (captured by DA_mask × shock_on in AC term).
    """
    N = geom['n_total']
    alpha_p = geom['alpha_p']
    alpha_m = geom['alpha_m']
    bouton_mask = geom['bouton_mask']
    stim_mask = geom['stim_mask']       # 1.0 for ALL boutons (Ca²⁺ from odour)
    da_mask = geom['da_mask']           # [1, 1, 0.5, 0, 0] at boutons
    ...
    k_on        = params[3]
    V_basal     = params[4]
    V_Ca        = params[5]     # Ca²⁺-only rutabaga rate (small)
    K_Ca        = params[6]
    n_Hill      = params[7]
    k_cat       = params[8]
    K_m         = params[9]
    ...
    tau_Ca      = params[13]
    Ca_dur      = params[14]
    V_coinc     = params[18]    # Coincidence rate (large, supralinear)
    ...
    def vector_field(t, y, args):
        ...
        # Ca²⁺/CaM activation function (shared by both terms)
        f_ca = ca**n_Hill / (K_Ca**n_Hill + ca**n_Hill + 1e-30)

        odor_on = odor_fn(t)
        shock_on = shock_fn(t)

        # Term 1: Ca²⁺-only rutabaga activation (all boutons during odour)
        #   Odour depolarises KC → Ca²⁺ → CaM → partial rutabaga
        #   Small but non-zero; present in ALL compartments
        j_ac_ca = bouton_mask * V_Ca * f_ca * odor_on

        # Term 2: Coincidence detection (Ca²⁺ + DA, compartment-specific)
        #   Shock → DANs → DA → Gαs + concurrent Ca²⁺/CaM → supralinear
        #   Only where DA is present (γ1-γ2 full, γ3 partial, γ4-γ5 zero)
        #   Requires Ca²⁺ (from ongoing odour) AND DA (from shock)
        j_ac_coinc = da_mask * V_coinc * f_ca * shock_on

        # Total AC: basal + Ca²⁺-only + coincidence
        ...
        # ---- Calcium ----
        # Ca²⁺ influx driven by ODOUR (KC depolarisation from odour-evoked
        # activity), NOT by shock. The shock activates DANs, not KCs.
        # All boutons receive Ca²⁺ because the odour depolarises the
        # entire KC axon (5-HT1B-GAL4 labels all γ-lobe KCs).
        j_ca_influx = stim_mask * odor_on * Ca_amp / Ca_dur
...
def sensor_lowpass(signal, dt, tau_sensor):
    ...
    G-Flamp1 in vitro kinetics (Wang et al. 2022):
      t_on  ≈ 0.20 s
      t_off ≈ 0.087 s
    In vivo response is slower due to cellular environment,
    but bounds are tightened to [0.5, 10] s to preserve the
    individual shock bumps visible in raw data at 5 s ITI.
...
def plot_diagnostic(params_array, data_full, pairing_onsets, geom,
    ...
    fig.suptitle('cAMP Model v3 — Two-Term AC (Ca²⁺-only + Coincidence)', fontsize=13, fontweight='bold')
...
def main():
    parser = argparse.ArgumentParser(
        description='GPU cAMP model optimisation v3 (two-term AC)')
    ...
    # Weights: upweight γ4-γ5 to capture the spatial gradient
    # These compartments have the most informative signal (DA-free zone)
    ...
    print(f"\n{'='*60}")
    print(f"GPU DIFFERENTIAL EVOLUTION v3 — Two-Term AC Model")
    print(f"{'='*60}")
    ...
    print(f"  ↳ τ_sensor bounds: [{float(FIT_BOUNDS[-1,0])}, {float(FIT_BOUNDS[-1,1])}] s")
