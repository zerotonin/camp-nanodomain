#!/usr/bin/env python3
"""
cAMP Nanodomain Model — GPU-Accelerated Parameter Optimisation v5c
====================================================================

MULTI-COMPONENT COST FUNCTION (Strategy B)

The base χ²/SEM cost allows a degenerate V_basal-only solution because
it treats all data points equally. This version adds three penalty terms
that enforce features the data clearly shows but V_basal cannot produce:

  Penalty 1 — COMPARTMENT GRADIENT (data feature, Level 1):
    At late timepoints (t ∈ [45,60]s), the data shows γ2 KD >> γ4-γ5 KD.
    V_basal is identical in all compartments and cannot produce this.
    Penalty: MSE between model's and data's per-compartment means at
    late timepoints, for both genotypes.

  Penalty 2 — GENOTYPE DIFFERENCE (known genetics, Level 3):
    The difference KD − WT can ONLY come from PDE (the dunce mutation).
    V_basal cancels in the difference. Penalty: MSE between model's and
    data's genotype difference (KD − WT) per compartment at late times.

  Penalty 3 — PRE-STIMULUS FLATNESS (experimental design, Level 2):
    Before t=0, no stimulus has been delivered. The signal should be flat.
    V_basal (without PDE in KD) produces a ramp even before t=0.
    Penalty: variance of model prediction in t ∈ [−5, 0] window.

  Total cost = χ²_base + w₁×comp_gradient + w₂×geno_diff + w₃×flatness

  All penalties reflect verifiable facts in the data or experimental design.
  None tell the model HOW to produce these features.

  Wide bounds retained from v5b. 12 free parameters.

Usage:
    python optimise_model_gpu_v5c.py --data all_camp_long.csv --outdir results_v5c/
    python optimise_model_gpu_v5c.py --data all_camp_long.csv --quick
    python optimise_model_gpu_v5c.py --plot-only results_v5c/best_params.json
"""
...
FIT_BOUNDS = jnp.array([
    [0.001,   50.0],   # V_basal       wide open
    [0.01,   200.0],   # V_Ca          wide open
    [0.1,    500.0],   # V_coinc       wide open
    [0.1,    500.0],   # k_cat         wide open
    [0.01,    50.0],   # PDE_conc      wide open
    [0.01,     5.0],   # K_Ca          very wide: 10 nM to 5 μM
    [0.1,    200.0],   # Ca_odor       wide open
    [0.1,    500.0],   # Ca_shock      wide open
    [0.1,    500.0],   # B_total       wide open
    [0.0001,  10.0],   # alpha         wide open
    [-2.0,     2.0],   # dff_offset    wide open
    [0.1,     30.0],   # tau_sensor    wide open
])
...
# =============================================================================
# 7. COST FUNCTION — MULTI-COMPONENT (Strategy B)
# =============================================================================

# Penalty weights — these can be tuned but modest values should suffice
# since V_basal literally cannot satisfy these penalties.
W_COMP_GRADIENT = 5.0    # compartment gradient (data feature)
W_GENO_DIFF     = 5.0    # genotype difference (genetics)
W_FLATNESS      = 10.0   # pre-stimulus flatness (experimental design)


def compute_late_time_targets(data, t_lo=45.0, t_hi=60.0):
    """Precompute data mean ΔF/F per compartment at late timepoints.

    Returns:
        late_wt: [5] mean ΔF/F per compartment for WT at t ∈ [t_lo, t_hi]
        late_kd: [5] mean ΔF/F per compartment for KD at t ∈ [t_lo, t_hi]
    """
    ...
    for i, comp in enumerate(comps):
        for geno, arr in [('dnc-wt', late_wt), ('dnc-KD', late_kd)]:
            key = (geno, comp)
            if key in data:
                t, m, _ = data[key]
                mask = (t >= t_lo) & (t <= t_hi) & np.isfinite(m)
                if mask.sum() > 0:
                    arr[i] = m[mask].mean()

    return jnp.array(late_wt), jnp.array(late_kd)


def cost_single(fit_values, geom, pairing_onsets, t_data_wt, t_data_kd,
                data_wt, data_kd, sem_wt, sem_kd, weights, t_save,
                late_wt_target, late_kd_target):
    """Multi-component cost for one parameter vector.

    Components:
      1. Base χ²/SEM:  standard fit to mean ± SEM (both genotypes)
      2. Compartment gradient: model's per-compartment late-time values
         must match data's per-compartment late-time values
      3. Genotype difference: (KD − WT) per compartment at late times
         must match data's genotype difference
      4. Pre-stimulus flatness: model prediction should be constant for t < 0
    """
    params = pack_full_params(fit_values)

    # --- Run both genotypes ---
    pred_wt = solve_forward(params, geom, pairing_onsets, 0.0, t_save)
    pred_kd = solve_forward(params, geom, pairing_onsets, 1.0, t_save)

    # =================================================================
    # Component 1: Base χ²/SEM (same as before)
    # =================================================================
    chi2_total = 0.0
    n_pts = 0.0

    for pred_dff, t_data, data_mean, data_sem in [
        (pred_wt, t_data_wt, data_wt, sem_wt),
        (pred_kd, t_data_kd, data_kd, sem_kd),
    ]:
        for k in range(5):
            pred_k = pred_dff[:, k]
            t_dk = t_data[k]
            ...
            resid = jnp.where(valid,
                              ...
                              0.0)
            chi2_total = chi2_total + jnp.sum(resid)
            n_pts = n_pts + jnp.sum(valid.astype(jnp.float64))

    cost_chi2 = chi2_total / jnp.maximum(n_pts, 1.0)

    # =================================================================
    # Component 2: Compartment gradient penalty
    # =================================================================
    # At late timepoints, the model's per-compartment predictions should
    # match the data's per-compartment values. This forces the model to
    # produce compartment differences that V_basal cannot.
    #
    # Use model predictions averaged over t ∈ [45, 60] s.
    late_mask = (t_save >= 45.0) & (t_save <= 60.0)
    n_late = jnp.sum(late_mask)

    # Model late-time means per compartment [5]
    model_late_wt = jnp.where(late_mask[:, None], pred_wt, 0.0).sum(axis=0) / jnp.maximum(n_late, 1.0)
    model_late_kd = jnp.where(late_mask[:, None], pred_kd, 0.0).sum(axis=0) / jnp.maximum(n_late, 1.0)

    # Penalty: MSE between model and data late-time compartment values
    cost_comp = (
        jnp.mean((model_late_wt - late_wt_target)**2) +
        jnp.mean((model_late_kd - late_kd_target)**2)
    )

    # =================================================================
    # Component 3: Genotype difference penalty
    # =================================================================
    # (KD − WT) per compartment at late times. V_basal cancels in this
    # difference, so the model MUST use PDE to produce it.
    model_diff = model_late_kd - model_late_wt     # [5]
    data_diff  = late_kd_target - late_wt_target    # [5]

    cost_geno = jnp.mean((model_diff - data_diff)**2)

    # =================================================================
    # Component 4: Pre-stimulus flatness penalty
    # =================================================================
    # Before t=0, no stimulus → signal should be flat (zero slope).
    # Penalise the variance of model predictions in t ∈ [−5, 0].
    pre_mask = (t_save >= -5.0) & (t_save < 0.0)
    n_pre = jnp.sum(pre_mask)

    pre_wt = jnp.where(pre_mask[:, None], pred_wt, 0.0)
    pre_kd = jnp.where(pre_mask[:, None], pred_kd, 0.0)

    # Variance across time for each compartment, then sum
    pre_mean_wt = pre_wt.sum(axis=0) / jnp.maximum(n_pre, 1.0)
    pre_mean_kd = pre_kd.sum(axis=0) / jnp.maximum(n_pre, 1.0)

    var_wt = jnp.where(pre_mask[:, None], (pred_wt - pre_mean_wt)**2, 0.0).sum(axis=0) / jnp.maximum(n_pre, 1.0)
    var_kd = jnp.where(pre_mask[:, None], (pred_kd - pre_mean_kd)**2, 0.0).sum(axis=0) / jnp.maximum(n_pre, 1.0)

    cost_flat = jnp.mean(var_wt) + jnp.mean(var_kd)

    # =================================================================
    # Total cost
    # =================================================================
    total = (cost_chi2
             + W_COMP_GRADIENT * cost_comp
             + W_GENO_DIFF * cost_geno
             + W_FLATNESS * cost_flat)
...
def plot_diagnostic(params_array, data_full, pairing_onsets, geom,
    ...
    fig.suptitle('cAMP Model v5c — Multi-Component Cost', fontsize=13, fontweight='bold')
...
def main():
    parser = argparse.ArgumentParser(
        description='GPU cAMP model optimisation v5c (multi-component cost)')
    ...
    # Precompute late-time targets for penalty terms
    late_wt_target, late_kd_target = compute_late_time_targets(data_full)
    print(f"\nLate-time targets (t ∈ [45,60]s mean ΔF/F):")
    for i, comp in enumerate(['γ1','γ2','γ3','γ4','γ5']):
        print(f"  {comp}:  WT={float(late_wt_target[i]):.3f}  "
              f"KD={float(late_kd_target[i]):.3f}  "
              f"diff={float(late_kd_target[i]-late_wt_target[i]):.3f}")
    print(f"  Compartment gradient (KD): "
          f"proximal={(float(late_kd_target[0])+float(late_kd_target[1]))/2:.3f}  "
          f"distal={(float(late_kd_target[3])+float(late_kd_target[4]))/2:.3f}")
    print(f"\nCost weights: χ²=1.0, comp_gradient={W_COMP_GRADIENT}, "
          f"geno_diff={W_GENO_DIFF}, flatness={W_FLATNESS}")
    ...
    print(f"\n{'='*60}")
    print(f"GPU DIFFERENTIAL EVOLUTION v5c — MULTI-COMPONENT COST")
    print(f"{'='*60}")
    print(f"Parameters ({len(FIT_NAMES)}): {FIT_NAMES}")
    print(f"  ↳ Cost = χ² + {W_COMP_GRADIENT}×comp_grad + {W_GENO_DIFF}×geno_diff + {W_FLATNESS}×flatness")
    print(f"  ↳ K_Ca:      [{float(FIT_BOUNDS[5,0])}, {float(FIT_BOUNDS[5,1])}] μM")
    ...
    # Bounds analysis: categorize each parameter
    print(f"\n  BOUNDS ANALYSIS:")
    print(f"  {'Parameter':>15s} {'Value':>10s} {'Lower':>10s} {'Upper':>10s} {'Rel.Pos':>10s} {'Status':>15s}")
    print(f"  {'-'*72}")
    for name, val in zip(FIT_NAMES, best_x):
        idx_in_fit2 = FIT_NAMES.index(name)
        lo2, hi2 = float(FIT_BOUNDS[idx_in_fit2, 0]), float(FIT_BOUNDS[idx_in_fit2, 1])
        rng = hi2 - lo2
        rel_pos = (val - lo2) / rng if rng > 0 else 0.5
        at_lo2 = rel_pos < 0.01
        at_hi2 = rel_pos > 0.99
        if at_lo2:
            status = "⬇ AT LOWER"
            ...
            status = "⬆ AT UPPER"
        elif rel_pos < 0.1 or rel_pos > 0.9:
            status = "⚠ NEAR BOUND"
        else:
            status = "✓ INTERIOR"
        print(f"  {name:>15s} {val:10.4f} {lo2:10.4f} {hi2:10.4f} {rel_pos:10.2%} {status:>15s}")
    ...
    # --- Cost decomposition for best solution ---
    print(f"\n  Cost decomposition:")
    # Re-run model for both genotypes to compute individual components
    t_save_np = np.arange(-5.0, 82.0, 0.5)
    _, pred_wt_np = run_model_np(full_params, pairing_onsets, 0.0, t_save_np, geom)
    _, pred_kd_np = run_model_np(full_params, pairing_onsets, 1.0, t_save_np, geom)

    # Late-time model values
    lt_mask = (t_save_np >= 45.0) & (t_save_np <= 60.0)
    model_late_wt_np = pred_wt_np[lt_mask].mean(axis=0)
    model_late_kd_np = pred_kd_np[lt_mask].mean(axis=0)

    comp_grad_wt = np.mean((model_late_wt_np - np.array(late_wt_target))**2)
    comp_grad_kd = np.mean((model_late_kd_np - np.array(late_kd_target))**2)
    comp_grad_total = comp_grad_wt + comp_grad_kd

    model_diff_np = model_late_kd_np - model_late_wt_np
    data_diff_np = np.array(late_kd_target) - np.array(late_wt_target)
    geno_diff_cost = np.mean((model_diff_np - data_diff_np)**2)

    pre_mask = (t_save_np >= -5.0) & (t_save_np < 0.0)
    flat_wt = np.mean(np.var(pred_wt_np[pre_mask], axis=0))
    flat_kd = np.mean(np.var(pred_kd_np[pre_mask], axis=0))
    flat_total = flat_wt + flat_kd

    print(f"    χ² base:             {best_cost - W_COMP_GRADIENT*comp_grad_total - W_GENO_DIFF*geno_diff_cost - W_FLATNESS*flat_total:.4f}")
    print(f"    Comp gradient:       {comp_grad_total:.6f}  (×{W_COMP_GRADIENT} = {W_COMP_GRADIENT*comp_grad_total:.4f})")
    print(f"    Genotype diff:       {geno_diff_cost:.6f}  (×{W_GENO_DIFF} = {W_GENO_DIFF*geno_diff_cost:.4f})")
    print(f"    Pre-stim flatness:   {flat_total:.6f}  (×{W_FLATNESS} = {W_FLATNESS*flat_total:.4f})")

    print(f"\n  Late-time values (model vs data):")
    for i, comp in enumerate(['γ1','γ2','γ3','γ4','γ5']):
        print(f"    {comp}:  WT model={model_late_wt_np[i]:.3f} data={float(late_wt_target[i]):.3f}  |  "
              f"KD model={model_late_kd_np[i]:.3f} data={float(late_kd_target[i]):.3f}")
    ...
    result = {
        ...
        'cost_weights': {
            'chi2': 1.0,
            'comp_gradient': float(W_COMP_GRADIENT),
            'geno_diff': float(W_GENO_DIFF),
            'flatness': float(W_FLATNESS),
        },
        'cost_components': {
            'comp_gradient_raw': float(comp_grad_total),
            'geno_diff_raw': float(geno_diff_cost),
            'flatness_raw': float(flat_total),
        },
        'late_time_targets': {
            'wt': [float(x) for x in late_wt_target],
            'kd': [float(x) for x in late_kd_target],
        },
        'history': history,
        'total_time_s': total_time,
        'device': str(jax.devices()[0]),
        'model_version': 'v5c_multi_component_cost',
    }
