#!/usr/bin/env python3
"""
cAMP Nanodomain Model — GPU-Accelerated Parameter Optimisation v5b
====================================================================

WIDE-BOUNDS EXPLORATION: Let the optimizer find its preferred solution
without biochemical constraints, then tighten bounds iteratively.

All bounds opened very wide. Same 12 free parameters as v5.
Goal: identify what the data actually demands before imposing biology.

Usage:
    python optimise_model_gpu_v5b.py --data all_camp_long.csv --outdir results_v5b/
    python optimise_model_gpu_v5b.py --data all_camp_long.csv --quick
    python optimise_model_gpu_v5b.py --plot-only results_v5b/best_params.json
"""
...
def plot_diagnostic(params_array, data_full, pairing_onsets, geom,
    ...
    fig.suptitle('cAMP Model v5b — Wide Bounds Exploration', fontsize=13, fontweight='bold')
...
def main():
    parser = argparse.ArgumentParser(
        description='GPU cAMP model optimisation v5b (wide-bounds exploration)')
    ...
    print(f"\n{'='*60}")
    print(f"GPU DIFFERENTIAL EVOLUTION v5b — WIDE BOUNDS EXPLORATION")
    print(f"{'='*60}")
    print(f"Parameters ({len(FIT_NAMES)}): {FIT_NAMES}")
    print(f"  ↳ ALL BOUNDS WIDE OPEN — let optimizer find its preferred solution")
    ...
    result = {
        ...
        'model_version': 'v5b_wide_bounds_exploration',
    }
